...
func getK8sMasterVars(cs *api.ContainerService) (map[string]interface{}, error) {
	...
	if cs.Properties.IsAzureStackCloud() && cs.Properties.OrchestratorProfile.IsAzureCNI() {
		azureStackCNIParams = ",' NETWORK_API_VERSION=',variables('apiVersionNetwork'),' SERVICE_MANAGEMENT_ENDPOINT=',variables('serviceManagementEndpoint'),' RESOURCE_MANAGER_ENDPOINT=',variables('resourceManagerEndpoint'),' ACTIVE_DIRECTORY_ENDPOINT=',variables('activeDirectoryEndpoint')"
	}
